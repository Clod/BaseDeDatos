...
2. Se han descartado de esta documentación las tablas intermedias ("staging tables" como MovDebugEventos y SentianceEventos) ya que son exclusivas del pipeline temporal de procesamiento y no almacenan datos definitivos para consumo.
3. Tipos de datos estandarizados al formato SQL Server (nvarchar, float, datetime, text).
4. **Mapeo de Rutas (UUID):** En la nueva columna de mapeo para el campo `viaje`, el UUID original se obtiene desde `Trip.canonical_transport_event_id` o `DrivingInsightsTrip.transport_event_id`. Ambos almacenan exactamente el mismo UUID del SDK; se elige uno u otro en la documentación para evitar cruces (`JOIN`) innecesarios según la tabla que se esté consultando.
...
La información para completar esta tabla se puede obtener a partir de dos tipos de evento: DrivingInsights para los desplazamientos que los generan (CAR, MOTOCYCLE y BUS) o de UserContextUpdate con con criteria CURRENTEVENT para el resto.


| Columna SQL     | Tipo de Dato | Descripción                                           | Observaciones                                                                         | Mapping Nuevo Esquema                       |
| --------------- | ------------ | ----------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------- |
| IdOffload       | int          | Log de ingesta del bucket/lote.                       | **Discontinuado**                                                                     | N/A                                         |
| FechaOffload    | date         | Fecha de procesamiento local en la BD.                | **Discontinuado**                                                                     | N/A                                         |
| usuario         | nvarchar     | Identificador del usuario Sentiance                   | Id de Sentiance                                                                       | `SdkSourceEvent.sentiance_user_id`          |
| viaje           | nvarchar     | Identificador único del viaje (UUID).                 | `Ver abajo`                                                                           | `Trip.canonical_transport_event_id`         |
| modotransporte  | nvarchar     | Medio de transporte inferido                          | (ej. Auto, Caminando, Colectivo, Moto, Subte, Desconocido, Tren, RUNNING, Bicicleta). | `Trip.transport_mode`                       |
| comienzo        | datetime     | Fecha y hora de inicio del viaje.                     | `Ver abajo`                                                                           | `Trip.start_time`                           |
| fin             | datetime     | Fecha y hora de fin del viaje.                        | `Ver abajo`                                                                           | `Trip.end_time`                             |
| duracion        | int          | Duración total del viaje en segundos.                 | `Ver abajo`                                                                           | `Trip.duration_in_seconds`                  |
| metadata        | text         | Metadatos adicionales; contenedor para payloads JSON. | Por el momento está todo en                                                           | `[]` vacío fijo                             |
| velocidadmaxima | float        | Velocidad máxima alcanzada durante el viaje.          | Por el momento está todo en 1                                                         | Calculada recorriendo `Trip.waypoints_json` |
...
Para los registros de tipo userContextUpdate (donde el criterio es CURRENTEVENT), la información de los viajes se encuentra dentro del objeto userContext, específicamente en el array de events.
...
| --------------- | ------------ | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------- |
| IdOffload       | int          | Log de ingesta del bucket/lote.                   | **Discontinuado**                                                                                                                                                                    | N/A                                                         |
| FechaOffload    | date         | Fecha de procesamiento local.                     | **Discontinuado**                                                                                                                                                                    | N/A                                                         |
| usuario         | nvarchar     | Identificador del usuario Sentiance               | Id de Sentiance                                                                                                                                                                      | `SdkSourceEvent.sentiance_user_id`                          |
| viaje           | nvarchar     | Identificador único del viaje.                    | Ver la explicación en la tabla Transporte, más arriba                                                                                                                                | `Trip.canonical_transport_event_id`                         |
| distanciam      | float        | Distancia total recorrida en metros.              | `Ver abajo`                                                                                                                                                                          | `Trip.distance`                                             |
| polyline        | text         | Recorrido codificado geomorfológicamente.         | De acuerdo con lo hablado con Mitul la forma actualizada de obtener las trayectorias es a partir de los waypoints. Ver abajo un ejemplo de código para pasar de waypoints a polyline | Procesado y generado vía script desde `Trip.waypoints_json` |
| puntosrecorrido | text         | Lista JSON de puntos crudos con latitud/longitud. | Se puede usar el array de waypoints                                                                                                                                                  | Extraído y volcado como array desde `Trip.waypoints_json`   |
| ubicacioninicio | text         | Ubicación inicial estimada (Lat/Lon).             | Actualmente no se popula pero se puede obtener del array de waypoints (ver función más abajo)                                                                                        | Primer elemento del array de `Trip.waypoints_json`          |
| ubicacionfin    | text         | Ubicación final estimada (Lat/Lon).               | Actualmente no se popula pero se puede obtener del array de waypoints (ver función más abajo)                                                                                        | Último elemento del array de `Trip.waypoints_json`          |
| maximavelocidad | float        | Máxima velocidad registrada en el trayecto.       | Se puede obtener del array de waypoints (ver función más abajo)                                                                                                                      | Mximo valor de velocidad iterando `Trip.waypoints_json`     |
...
| Columna SQL | Tipo de Dato | Descripción                              | Observaciones                   | Mapping Nuevo Esquema               |
| ----------- | ------------ | ---------------------------------------- | ------------------------------- | ----------------------------------- |
| id          | int          | Clave primaria autonumérica de la tabla. | **NUEVO** (Identity).           | IDENTITY propio BD                  |
| registro    | datetime     | Momento de cálculo e inserción en base.  | getdate()                       | getdate()                           |
| usuario     | nvarchar     | Identificador de usuario.                | Sentiance id                    | `SdkSourceEvent.sentiance_user_id`  |
| viaje       | nvarchar     | Identificador de viaje.                  | id del viaje                    | `Trip.canonical_transport_event_id` |
| ocupante    | varchar      | Rol inferido (ej. DRIVER, PASSENGER).    | occupantRole de DrivingInsights | `Trip.occupant_role`                |
...
Actualmente los eventos vienen en drivingeventsall.csv. Nosotros estamos sacando la información de los viajes a partir de DrivingInsights:
...
| usuario           | nvarchar     | Identificador de usuario.                                       | Sentiance id                                                                                       | `SdkSourceEvent.sentiance_user_id`                                                  |
| viaje             | nvarchar     | Identificador de viaje.                                         | Id del viaje                                                                                       | `Trip.canonical_transport_event_id`                                                 |
| aceleracion       | text         | Lista JSON de aceleraciones.                                    | Consolida múltiples filas en un simple array/objeto.                                               | Reconstruir JSON Array de `DrivingInsightsHarshEvent` (harsh_type = 'ACCELERATION') |
| usotelefono       | text         | Eventos registrados sobre uso general de teléfono móvil.        |                                                                                                    | Reconstruir JSON Array de `DrivingInsightsPhoneEvent`                               |
| curvas            | text         | Detalles de curvas bruscas tomadas.                             | Equivalente adaptado a text.                                                                       | Reconstruir JSON Array de `DrivingInsightsHarshEvent` (harsh_type = 'TURN')         |
| celularfijo       | text         | Interacciones con el teléfono cuando estaba fijo en soporte.    | Están todos en                                                                                     | `[]` vacío fijo                                                                     |
| frenado           | text         | Lista JSON de frenadas.                                         | Equivalente adaptado a text.                                                                       | Reconstruir JSON Array de `DrivingInsightsHarshEvent` (harsh_type = 'BRAKING')      |
| excesodevelocidad | text         | Registros desglosados de excesos de velocidad.                  | **REEMPLAZA** Se corrigió el nombre de columna para coincidir con el original exceso_de_velocidad. | Reconstruir JSON Array de `DrivingInsightsSpeedingEvent`                            |
| llamados          | text         | Eventos JSON de llamadas de voz detectadas.                     | Equivalente adaptado a text.                                                                       | Reconstruir JSON Array de `DrivingInsightsCallEvent`                                |
| pantalla          | text         | Detalles sobre toques o iteracción con la pantalla del celular. | Están todos en                                                                                     | `[]` vacío fijo                                                                     |
...
| IdOffload       | int          | Log de ingesta del bucket/lote.                               | **Discontinuado** | N/A                                                                                                  |
| FechaOffload    | date         | Fecha de procesamiento local.                                 | **Discontinuado** | N/A                                                                                                  |
| usuario         | nvarchar     | Identificador de usuario.                                     |                   | `SdkSourceEvent.sentiance_user_id`                                                                   |
| viaje           | nvarchar     | Identificador de viaje.                                       |                   | `Trip.canonical_transport_event_id`                                                                  |
| aceleracion     | text         | Lista JSON de aceleraciones verdaderamente fuertes.           |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsHarshEvent` (ACCELERATION) |
| usotelefono     | text         | Eventos registrados sobre uso severo del teléfono móvil.      |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsPhoneEvent`                |
| curvas          | text         | Detalles de curvas extremadamente bruscas tomadas gravadas.   |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsHarshEvent` (TURN)         |
| celularfijo     | text         | Interacciones severas y prolongadas en soporte.               |                   | `[]` vacío                                                                                           |
| frenado         | text         | Lista JSON de frenadas críticas de asfalto.                   |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsHarshEvent` (BRAKING)      |
| excesovelocidad | text         | Registros desglosados de excesos de alta velocidad continuos. |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsSpeedingEvent`             |
| llamados        | text         | Llamadas distraídas con factor multiriesgo.                   |                   | Filtrar con hardcodeo general desde array reconstruido de `DrivingInsightsCallEvent`                 |
| pantalla        | text         | Iteracción grave in-transit.                                  |                   | `[]` vacío                                                                                           |
...
| IdOffload    | int          | Log de ingesta del bucket/lote.                        | **Discontinuado**                | N/A                                      |
| FechaOffload | date         | Fecha de procesamiento local.                          | **Discontinuado**                | N/A                                      |
| usuario      | nvarchar     | Identificador de usuario de Sentiance                  |                                  | `SdkSourceEvent.sentiance_user_id`       |
| viaje        | nvarchar     | Identificador de viaje.                                |                                  | `DrivingInsightsTrip.transport_event_id` |
| legal        | float        | Puntuación de legalidad ("legalScore").                | legalScore de DrivingInsights    | `DrivingInsightsTrip.legal_score`        |
| suavidad     | float        | Puntuación global de suavidad ("smoothScore").         | smoothScore de DrivingInsights   | `DrivingInsightsTrip.smooth_score`       |
| atencion     | float        | Puntuación global de atención ("focusScore").          | focusScore de DrivingInsights    | `DrivingInsightsTrip.focus_score`        |
| promedio     | float        | Media o puntuación final consolidada ("overallScore"). | loverallScore de DrivingInsights | `DrivingInsightsTrip.overall_score`      |
...
| usuario           | nvarchar     | Identificador de usuario de Sentiance                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `SdkSourceEvent.sentiance_user_id`                                                                                           |
| viaje             | nvarchar     | Identificador de viaje.                                             |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `DrivingInsightsTrip.transport_event_id`                                                                                     |
| concentracion     | float        | Puntuación de concentración, relacionada a llamadas.                | Es identifica a PuntajesPrirmariosTr.atencion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `DrivingInsightsTrip.focus_score`                                                                                            |
| aceleracionfuerte | float        | Penalidad numérica por aceleraciones violentas.                     | 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `DrivingInsightsTrip.harsh_acceleration_score`                                                                               |
| frenadofuerte     | float        | Penalidad numérica por frenados bruscos excesivos.                  | 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `DrivingInsightsTrip.harsh_braking_score`                                                                                    |
| curvasfuertes     | float        | Penalidad numérica debido a giros fuertes.                          | 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `DrivingInsightsTrip.harsh_turning_score`                                                                                    |
| anticipacion      | float        | Sub-puntuación de la anticipación del chofer al frenar/acelerar.    | 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Fijo en -1                                                                                                                   |
| celularfijo       | int          | Penalización global por el uso de celular fijo validada en número.  | 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Fijo en -1                                                                                                                   |
| eventosfuertes    | int          | Penalización agregada / cuenta global referida a eventos violentos. | **Ver pestáña bug porque hay que corregirlo** harshevents del CSV se está redondeando a 1 ó 0 y se guarda en eventosfuertes de PuntajesSecundariosTr (también hay muchos casos con 1). [https://docs.sentiance.com/sentiance-insights/overview-of-sentiance-insights/driving-insights/driving-events-and-scores](https://docs.sentiance.com/sentiance-insights/overview-of-sentiance-insights/driving-insights/driving-events-and-scores) Todo parece indicar que es un cálculo del backend. Le mando la consulta a Mitul: [https://sentiance.slack.com/archives/C09ECDR1WJX/p1770134986293669](https://sentiance.slack.com/archives/C09ECDR1WJX/p1770134986293669) Hi @Claudio The harsh event is a combination of harsh acceleration, harsh braking, and harsh turning. This event was a legacy metric, as we did not have individual scores in our previous on-device solution. Now, we have all of them as separate scores. | `COUNT(*)` agrupado sobre `DrivingInsightsHarshEvent` para el ID de viaje correspondido o cálculo Legacy basado en metadata. |
...
| Columna SQL    | Tipo de Dato | Descripción                                                               | Observaciones | Mapping Nuevo Esquema                            |
| -------------- | ------------ | ------------------------------------------------------------------------- | ------------- | ------------------------------------------------ |
| Id             | int          | Clave primaria autonumérica.                                              |               | Autogenerado por BD                              |
| Registro       | datetime     | Fecha de recepción en base de datos.                                      |               | `getdate()`                                      |
| EventoId       | int          | Referencia al ID del evento.                                              |               | `VehicleCrashEvent.source_event_id`              |
| usuario        | varchar      | Identificador del usuario.                                                | .             | `SdkSourceEvent.sentiance_user_id`               |
| LeidoFechaHora | datetime     | Marca de tiempo de lectura.                                               |               | `VehicleCrashEvent.time`                         |
| json           | varchar      | Payload en bruto con los detalles de telemetría pre-choque y post-choque. |               | Obtenido desde `VehicleCrashEvent.metadata_json` |
...
| Columna SQL    | Tipo de Dato | Descripción                                          | Observaciones | Mapping Nuevo Esquema                                       |
| -------------- | ------------ | ---------------------------------------------------- | ------------- | ----------------------------------------------------------- |
| Id             | int          | Llave primaria autonumérica.                         |               | Autogenerado por BD                                         |
| Registro       | datetime     | Fecha de recepción/actualización del perfil.         |               | `getdate()`                                                 |
| EventoId       | int          | Referencia al evento o batch asociado.               |               | `UserProfile.source_event_id`                               |
| usuario        | varchar      | Identificador único del usuario global.              |               | `SdkSourceEvent.sentiance_user_id`                         |
| LeidoFechaHora | datetime     | Marca de tiempo de lectura u orquestación.           |               | `getdate()` / orquestación local                            |
| json           | varchar      | Payload con el perfil extendido arrojado por el SDK. |               | Todo el objeto JSON a partir de `UserProfile.metadata_json` |
