...
4. [UserMetadata](#4-usermetadata) - Metadatos personalizados por usuario.
5. [Trip](#5-trip) - Entidad centralizada de transportes y viajes.

### Etapa 3: Seguridad y Driving Insights
6. [DrivingInsightsTrip](#6-drivinginsightstrip) - Puntuaciones de seguridad del viaje.
7. [DrivingInsightsHarshEvent](#7-drivinginsightsharshevent) - Maniobras bruscas (aceleración, frenado, giro).
8. [DrivingInsightsPhoneEvent](#8-drivinginsightsphoneevent) - Uso físico del teléfono al conducir.
9. [DrivingInsightsCallEvent](#9-drivinginsightscallevent) - Llamadas telefónicas en movimiento.
10. [DrivingInsightsSpeedingEvent](#10-drivinginsightsspeedingevent) - Intervalos de exceso de velocidad.
11. [DrivingInsightsWrongWayDrivingEvent](#11-drivinginsightswrongwaydrivingevent) - Conducción a contramano.

### Etapa 4: Contexto de Usuario y Localizaciones
12. [UserContextHeader](#12-usercontextheader) - Cabecera de cambios de contexto.
13. [UserContextUpdateCriteria](#13-usercontextupdatecriteria) - Criterios de actualización de contexto.
14. [UserHomeHistory](#14-userhomehistory) - Registro de venues clasificados como "Casa".
15. [UserWorkHistory](#15-userworkhistory) - Registro de venues clasificados como "Trabajo".
16. [UserContextActiveSegmentDetail](#16-usercontextactivesegmentdetail) - Segmentos de perfil activos del usuario.
17. [UserContextSegmentAttribute](#17-usercontextsegmentattribute) - Atributos numéricos de los segmentos.
18. [UserContextEventDetail](#18-usercontexteventdetail) - Historial detallado de eventos de contexto (`STATIONARY`, `IN_TRANSPORT`).

### Etapa 5: Históricos del SDK y Eventos Críticos
19. [TimelineEventHistory](#19-timelineeventhistory) - Historial unificado de la línea de tiempo del SDK.
20. [UserActivityHistory](#20-useractivityhistory) - Resúmenes simplificados de actividades (`TRIP`, `STATIONARY`).
21. [TechnicalEventHistory](#21-technicaleventhistory) - Logs técnicos, de soporte y de canalización.
22. [VehicleCrashEvent](#22-vehiclecrashevent) - Detección de colisiones/choques vehiculares graves.
23. [SdkStatusHistory](#23-sdkstatushistory) - Logs de estado del SDK y permisos móviles del usuario.
...
### 1. SentianceEventos
Tabla de aterrizaje cruda donde se insertan los payloads JSON de eventos recibidos por el webhook de Sentiance antes de cualquier normalización.
...
| `sentianceid` | `VARCHAR(64)` | NULL | *Ninguno* | ID único del evento generado por la API o plataforma de Sentiance. |
| `json` | `NVARCHAR(MAX)` | NULL | *Ninguno* | Payload original del evento JSON completo en formato de texto. |
| `tipo` | `VARCHAR(32)` | NULL | *Ninguno* | Tipo de evento recibido (ej. `TRIP_FINISHED`, `USER_CONTEXT_UPDATE`). |
...
| `procesado` | `BIT` | NULL | `0` | Flag secundario para compatibilidad de estados del pipeline de extracción. |
...
### 3. SdkSourceEvent
Actúa como índice maestro y puente de auditoría relacional, vinculando cada registro crudo de `SentianceEventos` con los registros normalizados en las tablas de dominio.
...
| `record_type` | `VARCHAR(32)` | NULL | *Ninguno* | Clasificación del registro procesado (ej. `DrivingInsights`, `Timeline`). |
| `sentiance_user_id` | `VARCHAR(64)` | NULL | *Ninguno* | Identificador único provisto por Sentiance para el usuario de la app. |
| `source_time` | `DATETIME2(3)` | NULL | *Ninguno* | Fecha y hora de creación del evento en el origen (servidor de Sentiance). |
| `source_event_ref` | `VARCHAR(64)` | NULL | *Ninguno* | Identificador de referencia externa provisto por la plataforma Sentiance. |
...
### 4. UserMetadata
Permite asociar campos personalizados o metadatos de configuración de negocio a cada ID de usuario registrado en la base.
...
| `first_seen_from` | `VARCHAR(32)` | NULL | *Ninguno* | Origen de la primera detección del viaje (ej. `Timeline` o `DrivingInsights`). |
...
| `is_provisional` | `BIT` | NULL | `0` | Flag que indica si el viaje es provisional (actualizado dinámicamente por SDK). |
| `transport_tags_json` | `VARBINARY(MAX)` | NULL | *Ninguno* | JSON serializado binario de etiquetas contextuales (ej. tags de autopistas, congestión). |
...
| `trip_id` | `BIGINT` | NULL | *Ninguno* | Enlace relacional directo al ID del viaje en la tabla `Trip` (si coincide). |
...
| `transport_tags_json` | `VARBINARY(MAX)` | NULL | *Ninguno* | Etiquetas específicas de infraestructura vial comprimidas. |
...
| `magnitude` | `NUMERIC(6,3)` | NULL | *Ninguno* | Magnitud de aceleración (frecuentemente expresada en fuerzas G o m/s²). |
| `confidence` | `NUMERIC(5,3)` | NULL | *Ninguno* | Probabilidad matemática de que el evento sea real (0.000 a 1.000). |
...
| `end_time_epoch` | `BIGINT` | NULL | *Ninguno* | Unix timestamp de finalización. |
| `waypoints_json` | `VARBINARY(MAX)` | NULL | *Ninguno* | Tramo geográfico del exceso de velocidad en binario. |
...
| `waypoints_json` | `VARBINARY(MAX)` | NULL | *Ninguno* | Trayectoria del segmento a contramano en formato binario. |
...
| `context_source_type` | `VARCHAR(32)` | NULL | *Ninguno* | Origen del contexto (ej. `API`, `SCHEDULED_POLL`, `FOREGROUND_SERVICE`). |
| `semantic_time` | `VARCHAR(32)` | NULL | *Ninguno* | Categoría de tiempo semántico (ej. `WEEKEND_EVENING`, `WORK_HOURS`). |
...
| `criteria_code` | `VARCHAR(32)` | NULL | *Ninguno* | Código técnico del gatillador (ej. `STATIONARY_TIME_ELAPSED`, `SIGNIFICANT_MOVEMENT`). |
...
| `venue_type` | `VARCHAR(32)` | NULL | *Ninguno* | Clasificación del establecimiento (`HOME`). |
...
| `venue_type` | `VARCHAR(32)` | NULL | *Ninguno* | Clasificación (`WORK`). |
...
Desglosa el array `events` presente en los payloads de cambio de contexto del usuario. Útil para registrar lapsos estacionarios y de viaje cortos.
...
Resumen simplificado y de baja latitud diseñado para dashboards ágiles, compilando estados generales de viaje o visitas estacionarias.
...
| `detection_status` | `VARCHAR(32)` | NULL | *Ninguno* | Estado operativo de detección (ej. `DETECTING`, `STANDBY`). |
