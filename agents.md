...
## 3. Surgical Precision (MANDATORY)
1. **Bypass Formatting Tools**: You MUST use a method that bypasses all automatic formatting tools to ensure that not a single blank line or space outside the target logic is touched.
2. **Atomic Changes**: Only modify the lines directly related to the task.

## 4. Git Protocol (MANDATORY)
